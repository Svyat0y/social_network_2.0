...
const Pagination = ({ onPageChanged, totalCount, pageSize, currentPage, portionSize = 10 }) => {
	...
	return (
		<div className={ style.pagination_wrapper }>
			...
			{ <button disabled={ portionNumber === 1 && true } className={ style.prevNextBtn } onClick={ () => {
			...
			{ <button disabled={ portionNumber === portionCount && true } className={ style.prevNextBtn } onClick={ () => {
				setPortionNumber(portionNumber + 1)
			} }>Next</button> }
		</div>
	);
